...
public class CameraController {
	...
	protected void rotate(Matrix4 parentWorld, Matrix4 transformation, Vector3 rotation) {
		...
		rotation.x = (float)(rotation.x * Math.PI / 180);
		rotation.y = (float)(rotation.y * Math.PI / 180);
		rotation.z = (float)(rotation.z * Math.PI / 180);
		Matrix4 M = Matrix4.createRotationX(rotation.x);
		M.mulAfter(Matrix4.createRotationY(rotation.y));
		M.mulAfter(Matrix4.createRotationZ(rotation.z));
		if (orbitMode) {
			Vector3 origin = new Vector3(0,0,0);
			transformation.clone().invert().mulPos(origin);
			M.mulBefore(Matrix4.createTranslation(origin.clone().negate()));
			M.mulAfter(Matrix4.createTranslation(origin));
		}
		transformation.mulBefore(M);
	...
	protected void translate(Matrix4 parentWorld, Matrix4 transformation, Vector3 motion) {
		// TODO#A3 SOLUTION START
		parentWorld.mulPos(motion);
		Matrix4 M = Matrix4.createTranslation(motion);	
		transformation.mulBefore(M);
		// SOLUTION END
	}
}
